...
public class Checkpoint : MonoBehaviour
{
    ...
    [Tooltip("If true, this checkpoint is used as the initial spawn and disables effects on spawn.")]
    [SerializeField]
    ...
    void Start()
    {
        ...
        {
            Debug.Log($"[Checkpoint] Auto-registering {gameObject.name} with CheckpointManager");
    ...
    void Update()
    {
        ...
        if (isLerpingToActivated)
        {
            ...
            UpdateGlow(currentColor);
            Debug.Log($"[Checkpoint] {gameObject.name} lerping to activatedColor: {activatedColor}");
    ...
    {
        if (!isActivated && other.CompareTag("Player") && CheckpointManager.Instance != null && CheckpointManager.Instance.GetCurrentCheckpoint() != transform)
        {
            Debug.Log($"[Checkpoint] Notifying CheckpointManager of activation for {gameObject.name} via trigger");
    ...
    public void Activate()
    {
        ...
        Debug.Log($"[Checkpoint] {gameObject.name} activated! Starting lerp to {activatedColor}");
        if (activationEffect != null)
        {
            Debug.Log($"[Checkpoint] Playing activation effect for {gameObject.name}");
            ...
            {
                Debug.LogWarning($"[Checkpoint] activationEffect on {gameObject.name} does not have a ParticleSystem component.");
            }
        }
        else
        {
            Debug.LogWarning($"[Checkpoint] No activationEffect assigned for {gameObject.name}");
        }
        if (secondaryActivationEffect != null)
        {
            Debug.Log($"[Checkpoint] Playing secondary activation effect for {gameObject.name}");
            ...
            {
                Debug.LogWarning($"[Checkpoint] secondaryActivationEffect on {gameObject.name} does not have a ParticleSystem component.");
            }
        }
    ...
    public void DisableEffectsForSpawn()
    {
        ...
        isActivated = true;
        isLerpingToActivated = false;
        // Optionally, set the color to activatedColor immediately
        if (runeMat != null)
        {
            currentColor = activatedColor;
            UpdateGlow(currentColor);
    ...
    void UpdateGlow(Color glowColor)
    {
        ...
        Debug.Log($"[Checkpoint] {gameObject.name} emission color updated to {glowColor * glowIntensity}");
    }
}
